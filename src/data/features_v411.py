"""FeaturesV411: features_v411 module v1.
...
@dataclass
class FeaturesV411Config:
    """Configuration for features_v411 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV411(nn.Module):
    """Neural network module for features_v411 v1.
    ...
    def __init__(self, config: Optional[FeaturesV411Config] = None):
        super().__init__()
        self.config = config or FeaturesV411Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV411 v1 with %d params",
