"""FeaturesV441: features_v441 module v1.
...
@dataclass
class FeaturesV441Config:
    """Configuration for features_v441 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class FeaturesV441(nn.Module):
    """Neural network module for features_v441 v1.
    ...
    def __init__(self, config: Optional[FeaturesV441Config] = None):
        super().__init__()
        self.config = config or FeaturesV441Config()
        c = self.config
        ...
        logger.info("Initialized FeaturesV441 v1 with %d params",
