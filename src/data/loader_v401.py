"""LoaderV401: loader_v401 module v2.
...
@dataclass
class LoaderV401Config:
    """Configuration for loader_v401 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class LoaderV401(nn.Module):
    """Neural network module for loader_v401 v2.
    ...
    def __init__(self, config: Optional[LoaderV401Config] = None):
        super().__init__()
        self.config = config or LoaderV401Config()
        c = self.config
        ...
        logger.info("Initialized LoaderV401 v2 with %d params",
