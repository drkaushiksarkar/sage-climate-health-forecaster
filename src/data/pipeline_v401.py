"""PipelineV401: pipeline_v401 module v1.
...
@dataclass
class PipelineV401Config:
    """Configuration for pipeline_v401 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class PipelineV401(nn.Module):
    """Neural network module for pipeline_v401 v1.
    ...
    def __init__(self, config: Optional[PipelineV401Config] = None):
        super().__init__()
        self.config = config or PipelineV401Config()
        c = self.config
        ...
        logger.info("Initialized PipelineV401 v1 with %d params",
