"""TransformsV411: transforms_v411 module v2.
...
@dataclass
class TransformsV411Config:
    """Configuration for transforms_v411 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class TransformsV411(nn.Module):
    """Neural network module for transforms_v411 v2.
    ...
    def __init__(self, config: Optional[TransformsV411Config] = None):
        super().__init__()
        self.config = config or TransformsV411Config()
        c = self.config
        ...
        logger.info("Initialized TransformsV411 v2 with %d params",
