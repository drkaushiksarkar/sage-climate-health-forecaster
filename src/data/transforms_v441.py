"""TransformsV441: transforms_v441 module v2.
...
@dataclass
class TransformsV441Config:
    """Configuration for transforms_v441 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class TransformsV441(nn.Module):
    """Neural network module for transforms_v441 v2.
    ...
    def __init__(self, config: Optional[TransformsV441Config] = None):
        super().__init__()
        self.config = config or TransformsV441Config()
        c = self.config
        ...
        logger.info("Initialized TransformsV441 v2 with %d params",
