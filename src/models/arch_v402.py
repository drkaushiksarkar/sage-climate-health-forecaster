...
@dataclass
class ArchV402Config:
    """Configuration for arch_v402 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class ArchV402(nn.Module):
    """Neural network module for arch_v402 v1.
    ...
    def __init__(self, config: Optional[ArchV402Config] = None):
        super().__init__()
        self.config = config or ArchV402Config()
        c = self.config
        ...
        logger.info("Initialized ArchV402 v1 with %d params",
