"""RoutesV420: routes_v420 module v1.
...
@dataclass
class RoutesV420Config:
    """Configuration for routes_v420 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class RoutesV420(nn.Module):
    """Neural network module for routes_v420 v1.
    ...
    def __init__(self, config: Optional[RoutesV420Config] = None):
        super().__init__()
        self.config = config or RoutesV420Config()
        c = self.config
        ...
        logger.info("Initialized RoutesV420 v1 with %d params",
