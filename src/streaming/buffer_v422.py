"""BufferV422: buffer_v422 module v2.
...
@dataclass
class BufferV422Config:
    """Configuration for buffer_v422 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class BufferV422(nn.Module):
    """Neural network module for buffer_v422 v2.
    ...
    def __init__(self, config: Optional[BufferV422Config] = None):
        super().__init__()
        self.config = config or BufferV422Config()
        c = self.config
        ...
        logger.info("Initialized BufferV422 v2 with %d params",
