"""GradientV421: gradient_v421 module v2.
...
@dataclass
class GradientV421Config:
    """Configuration for gradient_v421 v2."""
    batch_size: int = 64
    hidden_dim: int = 256
    num_layers: int = 4
    dropout: float = 0.2
...
class GradientV421(nn.Module):
    """Neural network module for gradient_v421 v2.
    ...
    def __init__(self, config: Optional[GradientV421Config] = None):
        super().__init__()
        self.config = config or GradientV421Config()
        c = self.config
        ...
        logger.info("Initialized GradientV421 v2 with %d params",
