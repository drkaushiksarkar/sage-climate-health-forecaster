...
@dataclass
class LoopV433Config:
    """Configuration for loop_v433 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class LoopV433(nn.Module):
    """Neural network module for loop_v433 v1.
    ...
    def __init__(self, config: Optional[LoopV433Config] = None):
        super().__init__()
        self.config = config or LoopV433Config()
        c = self.config
        ...
        logger.info("Initialized LoopV433 v1 with %d params",
