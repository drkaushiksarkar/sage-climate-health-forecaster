"""IndexingTransformsV1S20: indexing_transforms_v1_s20 module v1.
...
@dataclass
class IndexingTransformsV1S20Config:
    """Configuration for indexing_transforms_v1_s20 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class IndexingTransformsV1S20(nn.Module):
    """Neural network module for indexing_transforms_v1_s20 v1.
    ...
    def __init__(self, config: Optional[IndexingTransformsV1S20Config] = None):
        super().__init__()
        self.config = config or IndexingTransformsV1S20Config()
        c = self.config
        ...
        logger.info("Initialized IndexingTransformsV1S20 v1 with %d params",
