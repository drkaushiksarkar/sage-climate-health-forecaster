"""MemoryV421: memory_v421 module v1.
...
@dataclass
class MemoryV421Config:
    """Configuration for memory_v421 v1."""
    batch_size: int = 32
    hidden_dim: int = 128
    num_layers: int = 3
    dropout: float = 0.1
...
class MemoryV421(nn.Module):
    """Neural network module for memory_v421 v1.
    ...
    def __init__(self, config: Optional[MemoryV421Config] = None):
        super().__init__()
        self.config = config or MemoryV421Config()
        c = self.config
        ...
        logger.info("Initialized MemoryV421 v1 with %d params",
